function [acc,acc_m,acc_f,acc_hcp,acc_m_hcp,acc_f_hcp,ss] = calc_age_reg(age_ul,FC_UKB,cov,FC1_regressed,FC2_regressed,FC3_regressed,FC4_regressed,Gender_hcp)
for rpt = 1:100
    clear FC_regressed
    ...
    ind_used = [i_male(ind_male(1:ss/2));i_female(ind_female(1:ss/2))];
    
    FC = FC_UKB(ind_used,:);    Gender = [cov.sex(ind_used)];    Age = [cov.age(ind_used)];
    
    % regressing out age effects for UK Biobank samples
    n_ROI = 94;    n_person = size(FC,1);    regresscov = [Age,Age.^2,Age.^3];
